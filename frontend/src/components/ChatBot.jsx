...
import { Sparkles, X, Send, Brain, MessageCircle, Download, RotateCcw, Plus } from 'lucide-react';
...
export default function ChatBot({ chatBotMode = null, setChatBotMode }) {
  ...
  // ── Download conversation as enhanced notes (.md) ──────────────────────────
  const downloadAsNotes = async () => {
    if (messages.length === 0) return;
    setNotesDownloading(true);

    // Derive topic from first user message (first 60 chars, sanitised for filename)
    ...
    try {
      // Build a well-structured markdown document from the conversation
      const convoText = messages
        .map(m => `**${m.role === 'user' ? 'You' : 'AI Assistant'}:** ${m.content}`)
        .join('\n\n');

      // Use AI to compile and enhance the conversation into structured notes
      const { callAI } = await import('../lib/aiService').then(m => {
        // We'll call via a one-shot prompt using the existing gemini proxy path
        return { callAI: null };
      });

      // Format conversation directly into clean notes (no extra AI call needed)
      const lines = [];
      ...
      // Group AI responses as the actual notes content
      let noteIndex = 1;
      for (let i = 0; i < messages.length; i++) {
        ...
        if (msg.role === 'user') {
          ...
          // Format AI response: convert bullet markers if plain text
  ...
  return (
    <>
      ...
      <div className={`chatbot-drawer ${open ? 'chatbot-drawer-open' : ''}`}>
        ...
        <div className="chatbot-header">
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginLeft: 'auto' }}>
            ...
            {/* Download notes button — only shown when there are messages */}
            {hasMessages && (
              <button
                className="chatbot-action-btn"
                onClick={downloadAsNotes}
                disabled={notesDownloading}
                title="Download conversation as notes"
              >
      ...
      <style>{`
        ...
        /* ── Action buttons (New Chat, Download) ── */
        .chatbot-action-btn {
        ...
        .chatbot-action-btn:hover { background: #EEF2FF; color: #4F46E5; border-color: #C7D2FE; }
        .chatbot-action-btn:disabled { opacity: 0.5; cursor: not-allowed; }
        [data-theme="dark"] .chatbot-action-btn { background: #1E293B; border-color: #334155; color: #94A3B8; }
        [data-theme="dark"] .chatbot-action-btn:hover { background: rgba(79,70,229,0.2); color: #818CF8; border-color: #4F46E5; }
        ...
        .chatbot-messages::-webkit-scrollbar-thumb { background: #CBD5E1; border-radius: 99px; }
        .chatbot-messages::-webkit-scrollbar-thumb:hover { background: #94A3B8; }
        [data-theme="dark"] .chatbot-messages::-webkit-scrollbar-thumb { background: #334155; }
        [data-theme="dark"] .chatbot-messages::-webkit-scrollbar-thumb:hover { background: #475569; }
