...
export const createProfileForOAuthUser = async (userId, email, profileData) => {
  const { error } = await supabase.from('profiles').upsert({
    ...
    // Super-specialization fields (DM/MCh/DNB/Fellowship)
    super_spec_type: profileData.super_spec_type || null,
    super_speciality: profileData.super_speciality || null,
    super_college: profileData.super_college?.trim() || null,
    super_place: profileData.super_place?.trim() || null,
    super_year: profileData.super_year ? Number(profileData.super_year) : null,
    fellowship_name: profileData.fellowship_name?.trim() || null,
    fellowship_institution: profileData.fellowship_institution?.trim() || null,
    fellowship_duration: profileData.fellowship_duration || null,
    role: 'doctor',
    status: 'pending',
    verified: false,
  }, { onConflict: 'id' })
